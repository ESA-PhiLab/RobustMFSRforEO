...
#### PIQ/SSIM: Single-Scale and Multi-Scale SSIM
...
def _multi_scale_ssim(x: torch.Tensor, y: torch.Tensor, kernel: torch.Tensor,
    ...
    for iteration in range(levels):
        ...
        ssim_val, cs = _ssim_per_channel(x, y, kernel=kernel, k1=k1, k2=k2)
        mcs.append(cs)
...
def _ssim_per_channel_complex(x: torch.Tensor, y: torch.Tensor, kernel: torch.Tensor, k1: float = 0.01,
                              k2: float = 0.03) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
...
def _multi_scale_ssim_complex(x: torch.Tensor, y: torch.Tensor,
    ...
    for iteration in range(levels):
        ...
        ssim_val, cs = _ssim_per_channel_complex(x, y, kernel=kernel, k1=k1, k2=k2)
        mcs.append(cs)
